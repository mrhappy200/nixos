...
in
{
  ...
  modifications = final: prev: {
    ...
    # The guacamole server doesn't handle OSC sequences correctly, this patch fixes that. (https://github.com/apache/guacamole-server/pull/613)
    guacamole-server = addPatches prev.guacamole-server [ ./guacamole-server.diff ];

    # https://github.com/ValveSoftware/gamescope/issues/1622
    gamescope = prev.gamescope.overrideAttrs (_: {
      NIX_CFLAGS_COMPILE = [ "-fno-fast-math" ];
    });
    #lutris = prev.bottles.override { buildInputs = pkgs: [pkgs.libunwind ]; };
  };
}
